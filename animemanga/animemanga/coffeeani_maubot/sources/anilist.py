...
from ..utils import maubot_embed_result

import logging
logger = logging.getLogger(__name__)

async def maubot_anilist_embeds(ctx, cmd, entered_title):
    results = await anilist_search_anime_manga(cmd, entered_title, isDiscord=False)
    if not results:
    ...
    for idx, em in enumerate(embed_data):
        embed = maubot_embed_result(em, COLOR_ANILIST, NAME_ANILIST, idx, idx_total)
        if em.get('studios'):
...
async def maubot_anilist_search_character(ctx, entered_title):
    ...
    if data is not None and len(data) > 0:
        ...
        for character in data:
            ...
            embed = Embed(title=format_name(character["name"]["first"], character["name"]["last"]))
            embed.url = link
            embed.color = 3447003
...
async def maubot_anilist_search_user(ctx, entered_title):
    variables = {"search": entered_title, "page": 1}
