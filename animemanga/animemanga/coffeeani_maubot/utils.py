...
def maubot_embed_result(result, color: str=None, service: str=None, idx: int=None, idx_total: int=None):
    try:
        em = result
        embed = Embed(title=em.get('title', str(None)))
...
def maubot_embed_source(source_being_loaded=None, color=None):
    if not source_being_loaded:
        no_results = Embed(description=f"No results found....")
        return no_results
    embed = Embed(description=f"Searching {str(source_being_loaded)}... ⏳")
    # if color:
    #     embed.color = discord.Colour.from_str(color)
    return embed

async def maubot_translate_deepl(self, text, source_lang="KO", target_lang="EN"):
    deepl_key = await self.config.get("deepl_api", None)
    if not deepl_key:
